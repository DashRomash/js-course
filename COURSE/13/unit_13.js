...
function f5(arr, block) {
    ...
    for (let key in arr) // цикл
        out += `${key} : ${arr[key]} <br>`;// формат вывода `${key} : ${arr[key]} <br>`;
    //
...
function f11() {
    for (let key in a11) {
        if (a111 in a11) {// смотрим, если ключи
            delete a11[a111];
        }
    }
    f5(a11, '.out-11');
...
function f12() {
    let b12 = +in12.value;// обрати внимание! важно перевести получаемый инпут из строки в число, а то ошибки не будет но, не работать
    console.log("Значение из input:", b12);
    for (let key in a12) {
        if (a12[key] === b12) { // смотрим, если значения!
            delete a12[key];
