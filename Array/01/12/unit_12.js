...
const f1 = () => {
    const b = a1.every(item => {
        if (a1_from < item && item < a1_to) {
            return true;
        }
...
const f2 = () => {
    const b = a2.every(item => {
        if (a2_from < item && item < a2_to) {
            return true;
        }
    });
    document.querySelector('.out-2').textContent = b;
}
...
const f3 = () => {
    const b3 = a3.every(item => {
        if (item.reduce((sum, item) => sum += item) > 20) {
            return true;
        }
    });
    document.querySelector('.out-3').textContent = b3;
}
